...
public class DirectoryFileProviderTest extends TestCase {

    public void testGetRecursiveDirectoryFiles() throws Exception {
        final URL url = getClass().getResource("Dir-1A");
        DirectoryFileProvider provider = new DirectoryFileProvider(new File(url.getPath()));
    ...
    public void testNotRecursiveDirectoryFiles() throws Exception {
        final URL url = getClass().getResource("Dir-1A");
        DirectoryFileProvider provider = new DirectoryFileProvider(new File(url.getPath()), false);
        final List<File> allFile = provider.getAllFile();
